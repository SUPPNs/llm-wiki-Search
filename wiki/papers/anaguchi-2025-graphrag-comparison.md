...
- [[papers/arakaki-2026-bipartite-graph-rag-cosmetics|BGR による推薦理由研究]] と対照すると、内容関係のグラフと行動関係のグラフを組み合わせて根拠を補完する研究方向が考えられる。
...
- [[questions/integrating-behavior-and-knowledge-graphs|行動二部グラフと知識グラフをどのように統合するか]] - BGR 論文との比較から生じる統合課題。
...
- [[comparisons/graph-augmented-rag-objectives|グラフ拡張 RAG の目的と評価の比較]] - BGR 論文との横断比較。
- [[themes/hybrid-graph-rag-for-grounded-recommendations|根拠付き推薦のためのハイブリッド Graph-RAG]] - 横断比較から生じるテーマ候補。

## 矛盾

- [[papers/arakaki-2026-bipartite-graph-rag-cosmetics|BGR 論文]] は異なるデータとユーザー評価目的を扱うため、本論文の QA 指標と数値を直接比較して方式の優劣を判断しない。

## 情報源

- `raw/papers/graphrag-comparison.pdf` - 取り込まれた原資料。読みやすい英数字ファイル名へ改名済み。
- `raw/papers/bipartite-graph-rag-cosmetics.pdf` - BGR との比較に用いた追加原資料。
